"""Private dual-T4 replication of the ARC-GPT2 quartet memory gate.
...
REPLICATIONS = (
    {"seed": 314159, "group_seed": 920000, "action": "A1"},
...
TEMP_ROOT = Path("/kaggle/temp/arc_gpt2_quartet_replication_gpu")
...
def main() -> None:
    ...
    all_pretrained_passed_by_80 = len(pretrained_steps) == len(REPLICATIONS) and all(
        step <= 80 for step in pretrained_steps
    )
    ...
    replication_passed = (
        ...
        and all_pretrained_passed_by_80
        and pretrained_median_t <= 50
    )
    scientific_gate = {
        "name": "quartet_replication_v1",
        "passed": replication_passed,
        "checks": {
            "all_three_pretrained_final_gates_passed": all_pretrained_final_passed,
            "all_three_pretrained_passed_by_step_80": all_pretrained_passed_by_80,
            "pretrained_median_T_at_most_50": pretrained_median_t <= 50,
        },
    ...
    sample_efficiency = {
        ...
        "censoring_note": (
            "Per preregistration, a non-pass by 100 steps receives T=110 for the "
    ...
    result = {
        ...
        "scope": (
            "Three-seed dual-T4 replication of the synthetic contrastive quartet gate; "
