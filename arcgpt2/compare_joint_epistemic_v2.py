"""Apply matched gates to one-checkpoint three-task GPT-2 runs."""
...
TASK_CONTROLS = {
    ...
    "contact_mechanics": ("amnesic", "precontact", "shuffled_contact"),
}
...
def _maximum_shared_level(
    ...
    available = [set(int(key) for key in _levels(summary, mode, task)) for mode in modes]
...
def _metric(summary: dict[str, Any], mode: str, task: str, level: int):
...
def compare(pretrained: dict[str, Any], random_init: dict[str, Any]) -> dict[str, Any]:
    ...
    for task, controls in TASK_CONTROLS.items():
        ...
        pretraining_gain = full["consistent_mass"] - random_full["consistent_mass"]
        task_gates = {
    ...
    for task in TASK_CONTROLS:
        gates[f"{task}:validation_cross_entropy_improves"] = (
    ...
    return {
        ...
        "scope": (
            "One-GPT-2 action, goal, and contact-mechanics epistemic gate. "
